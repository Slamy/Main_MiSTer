...
static int load_cue(const char* filename, toc_t* table)
{
	...
	FileOpen(&toc.sub, getFullPath(fname));
		...
		else
		{
			...
			for (int i = 0; i < toc.last; i++)
			{
				if (lba >= (toc.tracks[i].start - toc.tracks[i].pregap) && lba <= toc.tracks[i].end)
				{
					...
					while (cnt)
					{
						uint8_t subc[SUBCHANNEL_RW_SIZE];
						...
						else
						{
							...
							if (toc.sub.opened())
							{
								FileReadAdv(&toc.sub, subc, SUBCHANNEL_RW_SIZE);
								subc_filled = true;
								reinterleave_subcode = true;
							}
						}
						...
						{
							subcode_rw_byte_to_word_map(subc, subcode_out.rw);
						}
						else
						{
							// Force zero to avoid accidental CD+G detection in CD player
